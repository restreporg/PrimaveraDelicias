...
<section class="acerca">
  ...
  <div class="acerca__cards">
    <div class="card__cocineros">
      ...
      <h2 class="card-cocineros__titulo">
```
...
Esta sección aplica BEM de forma más canónica. `menu-card` es el bloque base compartido por las cuatro tarjetas. Los modificadores `menu-card--tacos`, `menu-card--sandwich`, `menu-card--pechuga` y `menu-card--bandeja` permiten personalizar cada tarjeta individualmente (por ejemplo, cambiar la imagen de fondo o el color de acento) sin duplicar los estilos base. Los elementos `menu-card__imagen`, `menu-card__titulo`, `menu-card__descripcion` y `menu-card__arrow` son consistentes en las cuatro tarjetas, lo que facilita el mantenimiento: si se quiere cambiar el estilo del título de todas las cards, basta con modificar un solo selector.
